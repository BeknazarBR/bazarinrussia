...
if(isAjax() == true){
   ...
   if( !$_POST["user_shop_discount_status"] ){
        $_POST["user_shop_discount_month"] = 0;
        $_POST["user_shop_discount_percent"] = 0;
   }
   ...
   update("UPDATE uni_settings SET value=? WHERE name=?", array(intval($_POST["authorization_option"]),'authorization_option'));
   ...
   update("UPDATE uni_settings SET value=? WHERE name=?", array(round($_POST["user_shop_price_month"],2),'user_shop_price_month'));
   update("UPDATE uni_settings SET value=? WHERE name=?", array(intval($_POST["user_shop_discount_month"]),'user_shop_discount_month'));
   update("UPDATE uni_settings SET value=? WHERE name=?", array(round($_POST["user_shop_discount_percent"],2),'user_shop_discount_percent'));
   ...
   update("UPDATE uni_settings SET value=? WHERE name=?", array(clear($_POST["format_phone"]),'format_phone'));
   ...
   update("UPDATE uni_settings SET value=? WHERE name=?", array(clear($_POST["social_link_fb"]),'social_link_fb'));
   update("UPDATE uni_settings SET value=? WHERE name=?", array(clear($_POST["social_link_inst"]),'social_link_inst'));
