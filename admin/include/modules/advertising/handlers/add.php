...
if(isAjax() == true){
 ...
 if($type_banner == 1){
     ...
     if(!$_FILES["image"]["name"]){
        $error[] = "Пожалуйста, выберите баннер";
     }
...
if (count($error) == 0) {
   ...
   if($_POST["type_cat"] == 1){
    ...
    insert("INSERT INTO uni_advertising(advertising_title,advertising_image,advertising_visible,advertising_ids_cat,advertising_type_banner,advertising_date_start,advertising_date_end,advertising_geo,advertising_banner_position,advertising_code_script,advertising_index_out,advertising_out_podcat,advertising_link_site,advertising_pages,advertising_var_out)VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", array($title,$image["name"],$visible,$ids_cat,$type_banner,$date_start,$date_end,$geo,$banner_position,$code_script,$index_out,$out_podcat,$link_site,implode(",", $_POST["pages"]),$var_out) );        
