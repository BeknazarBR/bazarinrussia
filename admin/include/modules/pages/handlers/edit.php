...
if (count($error) == 0) {
   ...
   update("UPDATE uni_pages SET name=?,title=?,alias=?,text=?,seo_text=?,visible=? WHERE id=?", array(clear($_POST["name"]),clear($_POST["title"]),$alias,$_POST["text"],$_POST["desc"],intval($_POST["visible"]),intval($_POST["id"])));    
