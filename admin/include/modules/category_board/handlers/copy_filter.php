       ...
       foreach ($_POST["value_filter"] as $action => $array) {
         foreach ($array as $id => $value) {
            ...
            if($action == "add" && trim($value)){
               insert("INSERT INTO uni_ads_filters_items(ads_filters_items_id_filter,ads_filters_items_value,ads_filters_items_alias)VALUES(?,?,?)", array( $insert, clear($value), translite($value) ));
            }
