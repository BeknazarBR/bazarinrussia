...
<div class="form-group row d-flex align-items-center mb-5">
  ...
  <div class="col-lg-9">
      <input type="text" name="name" class="form-control" value="<?php echo $getFilter->ads_filters_name; ?>" /> 
  </div>
</div>
...
<div class="form-group row d-flex align-items-center mb-5">
  ...
  <div class="col-lg-9">
      <input type="text" name="alias" class="form-control" value="<?php echo $getFilter->ads_filters_alias; ?>" /> 
  </div>
</div>
...
<div class="form-group row d-flex">
  ...
  <div class="col-lg-9">
   ...
   <a href="#" class="btn btn-success btn-sm action-add-item-filter" ><i class="la la-plus" ></i> Добавить значение</a>
   ...
   <div class="list-podfilter" >
      ...
      <?php
        ...
        $items = getAll("SELECT * FROM uni_ads_filters_items WHERE ads_filters_items_id_filter=? order by ads_filters_items_id asc", array($id_filter));
          if(count($items)>0){
           
             foreach($items AS $item){ 

                   ?>
                   <div class="podfilter-item" ><input type="text" class="form-control" value="<?php echo $item["ads_filters_items_value"]; ?>" name="value_filter[add][<?php echo $item["ads_filters_items_id"]; ?>]" /><i class="la la-times delete-podfilter" ></i></div>
                   <?php
