...
if (count($error) == 0) {
    ...
    if($_POST["value_filter"]){
       ...
       update("DELETE FROM uni_ads_filters_items WHERE ads_filters_items_id_filter='".$id_filter."' and ads_filters_items_id_item_parent='".$id_item."' and ads_filters_items_id NOT IN(".implode(",", $ids_delete).")");

    }else{
       update("DELETE FROM uni_ads_filters_items WHERE ads_filters_items_id_filter=? and ads_filters_items_id_item_parent=?", [$id_filter,$id_item]);
    }       
