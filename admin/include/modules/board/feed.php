...
if($get["count"]){
    ...
    foreach ($get["all"] as $key => $value) {
        ...
        "filters" => $properties,
        "user" => [ "name" => $value["clients_name"], "surname" => $value["clients_surname"], "avatar" => Exists($config["media"]["avatar"],$value["clients_avatar"],$config["media"]["no_avatar"]), "email" => $value["clients_email"], "phone" => $value["clients_phone"], "link" => _link("user/".$value["clients_id_hash"]), "shop" => $getShop ? 1 : 0, "link_shop" => $Shop->link($getShop["clients_shops_id_hash"]) ],
    	];
