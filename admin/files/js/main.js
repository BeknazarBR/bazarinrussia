$(document).ready(function() {
  ...
  function notifications_toolbar(){
    $.ajax({type: "POST",url: url_path + "systems/ajax/admin.php",data: "action=notifications_toolbar",dataType: "json",cache: false,
        success: function (data) {
          if(data["alert"]){
            $(".notification-alert .notification").html(data["alert"]); $(".notification-alert").show();
          }else{
  ...
  setInterval(notifications_toolbar, 5000);

  $(".cash-clear").click(function () {
    $.ajax({type: "POST",url: url_path + "systems/ajax/admin.php",data: "action=cash-clear",dataType: "html",cache: false,
        success: function (data) {
          location.reload();
        }
    });        
 ...
 $(document).on('change','.toolbat-toggle', function () {      
    $.ajax({type: "POST",url: url_path + "systems/ajax/admin.php",data: $(".quick-settings-form").serialize()+"&action=quick-settings",dataType: "html",cache: false,
        success: function (data) {}
    });   
 });
