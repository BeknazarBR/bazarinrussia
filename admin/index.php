...
if(!empty($_GET["open_toolbar"])){
   update("DELETE FROM uni_notifications WHERE id=?", array(intval($_GET["open_toolbar"])));
}

if( $_GET["route"] == "complaints" ){
  update("UPDATE uni_ads_complain SET ads_complain_status=?", array(1));
}

$getComplain = (int)getOne("select count(*) as total from uni_ads_complain where ads_complain_status='0'")["total"];
...
<head>
    ...
    <link href="<?php echo  $config["urlPath"] . "/" . $config["folder_admin"]; ?>/files/css/styles.css" rel="stylesheet" type="text/css" /> 
