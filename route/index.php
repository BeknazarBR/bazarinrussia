...
$data["vip"] = $Ads->getAll( ["query"=>"ads_status='1' and clients_status IN(0,1) and ads_period_publication > now() and ads_vip='1' $geo order by rand() limit 4", "param_search" => $param_search, "output" => 4 ] );
$data["shops"] = getAll( "select * from uni_clients_shops INNER JOIN `uni_clients` ON `uni_clients`.clients_id = `uni_clients_shops`.clients_shops_id_user where clients_shops_time_validity > now() and clients_status IN(0,1) order by rand() limit 3" );
$data["slider_ad_category"] = $Main->outSlideAdCategory();
