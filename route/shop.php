...
if( $data["current_page"] ){
    ...
    if( $data["current_category"] ){
        ...
        $data["h1"] = $ULang->t('Каталог объявлений');
...
$data["category"] = $Shop->outCategories( $Shop->adCategories( $data["shop"]["clients_shops_id_user"] ), 0, $data["shop"]["clients_shops_id_hash"],$data["current_category"]["category_board_id"] );
...
if( !$_SESSION['cp_auth'][ $config["private_hash"] ] ){
      ...
      if( $data["user"]["clients_status"] != 2 ){
          ...
          if( strtotime($data["shop"]["clients_shops_time_validity"]) > time() ){
...
$data["share"] = $Main->share( array( "title" => $data["shop"]["clients_shops_title"], "image" => Exists($config["media"]["other"], $data["shop"]["clients_shops_logo"], $config["media"]["no_image"]), "url" => $Shop->link( $data["shop"]["clients_shops_id_hash"] ), "description" => $data["shop"]["clients_shops_desc"] ) );
