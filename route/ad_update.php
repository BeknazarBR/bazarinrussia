...
if(count($getCategoryBoard["category_board_id_parent"][0])){
	foreach ($getCategoryBoard["category_board_id_parent"][0] as $key => $value) {
	  $data["list_categories"] .= '<span class="box-change-category-list-item" data-name="'.$value["category_board_name"].'" data-id="'.$key.'" >'.$value["category_board_name"].'</span>';
	}
}
...
echo $Main->tpl("ad_update.tpl", compact( 'Seo','Geo','Main','visible_footer','Ads','out_metro','out_area','route_name','Profile','CategoryBoard','getCategoryBoard','data','settings','list_period','Filters','Banners', 'ULang', 'dropdown_currency','Cart' ) );
