...
class Cart{
	...
	function returnAvailable($id){
		 ...
		 if($getOrder){
	         ...
	         $getAds = getAll('select * from uni_clients_orders_ads where clients_orders_ads_order_id=?', [$getOrder['clients_orders_uniq_id']]);

	         if(count($getAds)){
	             foreach ($getAds as $key => $value) {
	                 $get = findOne('uni_ads', 'ads_id=?', [ $value['clients_orders_ads_ad_id'] ]);
	                 if($get && !$get['ads_available_unlimitedly']){
	                    update('update uni_ads set ads_available=ads_available+'.$value['clients_orders_ads_count'].' where ads_id=?', [$value['clients_orders_ads_ad_id']]);
	                 }
	             }
	         }
