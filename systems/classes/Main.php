    ...
    function assets( $assets = array() ){
    	...
    	if(isset($assets)){
    		foreach ($assets as $key => $link) {
    			if(!is_array($link)){
                    echo str_replace( array("{urlPath}","{lang}","{templateFolder}"),array($config["urlPath"],getLang(),$config["template_folder"]), $link );
    			}else{
    				if($link[$route_name]){
                       echo str_replace( array("{urlPath}","{lang}","{templateFolder}"), array($config["urlPath"],getLang(),$config["template_folder"]), implode("", $link[$route_name]) );
    				}
    			}
    		}
    	}
    ...
    function share( $data = array() ){
        ...
        return '
           ...
           <a target="_blank" class="social-icon" href="https://connect.ok.ru/dk?st.cmd=WidgetSharePreview&st.shareUrl='.$data["url"].'&st.title='.$data["title"].'" >
             <img src="'.$config["urlPath"].'/'.$config["template_folder"].'/images/icon-ok.png" height="32" >
           </a>
           <a target="_blank" class="social-icon" href="http://www.facebook.com/sharer.php?p[url]='.$data["url"].'" >
             <img src="'.$config["urlPath"].'/'.$config["template_folder"].'/images/icon-fb.png" height="32" >
           </a>
        ';
    }
    ...
    function socialLink(){
       ...
       if($settings["social_link_fb"]){ $link .= '
           <a class="social-icon" href="'.$settings["social_link_fb"].'">
             <img src="'.$config["urlPath"].'/'.$config["template_folder"].'/images/icon-fb.png" >
           </a>
       '; }
       if($settings["social_link_inst"]){ $link .= '
           <a class="social-icon" href="'.$settings["social_link_inst"].'">
             <img src="'.$config["urlPath"].'/'.$config["template_folder"].'/images/icon-inst.png" >
           </a>
       '; }
