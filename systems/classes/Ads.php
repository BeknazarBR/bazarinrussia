...
class Ads{
        ...
        <div class="board-view-button">
            ...
            if($permission_add_cart){
                ...
                if($button){
                    ...
                    ?>
                      <div class="btn-custom btn-color-blue schema-color-button ad-add-to-cart mb5" data-name-add="<?php echo $ULang->t("Добавить в корзину"); ?>" data-name-delete="<?php echo $ULang->t("Удалить из корзины"); ?>" data-id="<?php echo $data["ad"]["ads_id"]; ?>" >
  ...
  function addSecurePayments( $data = [] ){
     ...
     insert("INSERT INTO uni_secure_payments(secure_payments_date,secure_payments_id_order,secure_payments_score,secure_payments_amount,secure_payments_status_pay,secure_payments_id_user,secure_payments_status,secure_payments_amount_percent)VALUES(?,?,?,?,?,?,?,?)", [ date("Y-m-d H:i:s"), $data["id_order"], $data["score"], $data["amount"], $data["status_pay"], $data["id_user"], $data["status"], $data["amount_percent"] ]);
  ...
  function secureResultPay( $data = [] ){
    ...
    $getPayment["secure_payments_score"] = decrypt($getPayment["secure_payments_score"]);

    if($getPayment){
        if($getPayment["secure_payments_status_pay"] == 0){

          ?>
          <p class="mt10"><?php echo $Main->price( $getPayment["secure_payments_amount_percent"] ); ?> <?php echo $ULang->t("будут зачислены на ваш счет в течении 24 часа."); ?></p>
          <?php

        }elseif($getPayment["secure_payments_status_pay"] == 1){
          
          ?>
          <p class="mt10"><?php echo $Main->price( $getPayment["secure_payments_amount_percent"] ); ?> <?php echo $ULang->t("зачислены на ваш счет"); ?>
          <?php 
          echo "xxxx" . substr($getPayment["secure_payments_score"], strlen($getPayment["secure_payments_score"]) - 4, strlen($getPayment["secure_payments_score"]) ); ?>
          </p>
          <?php
          ...
          ?>
          <p class="mt10"><?php echo $ULang->t("Не удалось перевести деньги на счет"); ?></p>
           
          <a class="btn-custom-mini btn-color-blue mt5 mb5" href="<?php echo _link("user/".$getUser["clients_id_hash"]."/settings"); ?>" > <span><?php echo $ULang->t("Указать другой счет"); ?></span> </a>
