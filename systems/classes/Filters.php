...
class Filters{
  ...
  function getCategory( $param = [] ){
   ...
   function checkSelected($id = 0,$id_item = 0, $param = []){
                  ...
                  if($getFilter){
                      foreach($array AS $val => $result){
                          ...
                          if($getFilter->ads_filters_type == "input"){
                             ...
                             if($getAlias){
                               $value[] = '<a title="'.$ULang->t( $getAlias["ads_filters_alias_title"] , [ "table" => "uni_ads_filters_alias", "field" => "ads_filters_alias_title" ] ).'" href="'.$this->alias( ["category_alias"=>$category["category_board_id"][$id_cat]["category_board_chain"], "filter_alias"=>$getAlias["ads_filters_alias_alias"], "geo_alias" => $city_alias] ).'" >'.$ULang->t( $getItem->ads_filters_items_value , [ "table" => "uni_ads_filters", "field" => "ads_filters_items_value" ] ).'</a>'; 
                             }else{
                      ...
                      $out[$getFilter->ads_filters_position][] = '<div class="col-lg-3 col-md-3 col-sm-5 col-5" ><span class="span-style" >'.$ULang->t( $getFilter->ads_filters_name , [ "table" => "uni_ads_filters", "field" => "ads_filters_name" ] ).'</span></div> <div class="col-lg-9 col-md-9 col-sm-7 col-7" > <span class="span-style-height" >'.implode(",", $value).'</span> </div>';
                  }
    ...
    function outProductPropArray($product_id=0, $id_cat=0, $category = []){
       ...
       if($search){
        ...
        if(count($split) > 0){
              foreach ($split as $key => $value) {
                $return[] = "(ads_title LIKE '%".clearText($value)."%' or ads_filter_tags LIKE '%".clearText($value)."%' or city_name LIKE '%".clearText($value)."%' or category_board_name LIKE '%".clearText($value)."%' or ads_address LIKE '%".clearText($value)."%')";
              }
              return implode(" AND ",$return);
        }else{
          return "(ads_title LIKE '%".clearText($search)."%' or ads_filter_tags LIKE '%".clearText($search)."%' or city_name LIKE '%".clearText($search)."%' or category_board_name LIKE '%".clearText($search)."%' or ads_address LIKE '%".clearText($search)."%')";
        }
       }
    }
