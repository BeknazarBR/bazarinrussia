  ...
  if($_POST["action"] == "update_user_chat"){
     ...
     if( $count ){
      ...
      if( $getUser && $getMessage ){
         ...
         if($getMessage["chat_messages_action"] == 0){
             $message = '
                <strong>'.$getUser["clients_name"].'</strong>
                <p>'.custom_substr(decrypt($getMessage["chat_messages_text"]), 100, "...").'</p>
             ';
         }elseif($getMessage["chat_messages_action"] == 1){
             $message = '
                <strong>'.$getUser["clients_name"].'</strong>
                <p>'.custom_substr($ULang->t("Покупатель добавил объявление в избранное. Договоритесь с ним о сделке в этом чате."), 100, "...").'</p>
             ';            
         }elseif($getMessage["chat_messages_action"] == 2){
             $message = '
                <strong>'.$getUser["clients_name"].'</strong>
                <p>'.custom_substr($ULang->t("Ваш номер просмотрели"), 100, "...").'</p>
             ';            
         }elseif($getMessage["chat_messages_action"] == 3){
             $message = '
                <strong>'.$getUser["clients_name"].'</strong>
                <p>'.custom_substr($ULang->t("Оформление заказа"), 100, "...").'</p>
             ';              
         }elseif($getMessage["chat_messages_action"] == 4){
             $message = '
                <strong>'.$getUser["clients_name"].'</strong>
                <p>'.custom_substr($ULang->t("У вас новый отзыв"), 100, "...").'</p>
             ';              
         }elseif($getMessage["chat_messages_action"] == 5){
             $message = '
                <strong>'.$getUser["clients_name"].'</strong>
                <p>'.custom_substr($ULang->t("Вы победили в аукционе! За вами закреплен данный лот, если оплата не поступит, то продавец вправе отменить сделку и запустить торги заново!"), 100, "...").'</p>
             ';              
         }elseif($getMessage["chat_messages_action"] == 6){
             $message = '
                <strong>'.$getUser["clients_name"].'</strong>
                <p>'.custom_substr($ULang->t("Ваша ставка перебита! Вы можете заново сделать ставку и побороться за данный лот!"), 100, "...").'</p>
             ';              
         }
