...
$cats = $CategoryBoard->getCategories("where category_board_visible=1");

if(count($getAds)){
   foreach ($getAds as $ad_value) {

      $ids_cat = explode(',', $CategoryBoard->reverseId($cats,$ad_value['ads_id_cat']));
      ...
      if( count($ids_cat) ){
          foreach ($ids_cat as $cat_value) {
              ...
              if($cat_value){
                 ...
                 $ads[ $cat_value ][ $ad_value['ads_country_id'] ] += 1;
                 $ads[ $cat_value ][ $ad_value['ads_region_id'] ] += 1;
                 $ads[ $cat_value ][ $ad_value['ads_city_id'] ] += 1;
