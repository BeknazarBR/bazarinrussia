...
if( count($cities) ){
	foreach ($cities as $id_city => $nested) {
			...
			foreach ($nested["ads"] as $value) {
			     ...
			     if( !findOne( "uni_city_distance", "(city_distance_id_city_from=? and city_distance_id_city_to=?) or (city_distance_id_city_to=? and city_distance_id_city_from=?)", [ $id_city, $value["ads_city_id"], $value["ads_city_id"], $id_city ] ) && $id_city != $value["ads_city_id"] ){
