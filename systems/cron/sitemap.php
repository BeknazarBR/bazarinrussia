...
if($settings["sitemap_category"]){
   ...
   if(count($getAds)){
      foreach ($getAds as $key => $ad_value) {
         ...
         if( $getCity && $getCategories["category_board_id"][$ad_value["ads_id_cat"]]["category_board_chain"] ){
               $lines .= '
                   <url>
...
if($settings["sitemap_shops"]){
   ...
   $getShops = getAll( "select * from uni_clients_shops INNER JOIN `uni_clients` ON `uni_clients`.clients_id = `uni_clients_shops`.clients_shops_id_user where clients_shops_time_validity > now() and clients_status IN(0,1)" );

   if($getShops){
      foreach ($getShops as $value) {
         $lines .= '
             <url>
              <loc>'.$Shop->link($value["clients_shops_id_hash"]).'</loc>
              <lastmod>'.date("Y-m-d").'</lastmod>
              <priority>0.6</priority>
             </url>
         ';
