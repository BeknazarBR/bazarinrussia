...
$getMessage = getAll("select * from uni_chat_messages where chat_messages_status=0 and chat_messages_notification=0 group by chat_messages_id_hash");
...
if(count($getMessage)){
   ...
   foreach ($getMessage as $key => $value) {
      ...
      $data = array("{PROFILE_LINK_CHAT}"=>_link("chat"),
...
if(count($getComments)){
   ...
   foreach ($getComments as $key => $value) {
      ...
      if($value["ads_comments_id_parent"]){
...
$getClientsSubscriptions = getAll("select * from uni_clients_subscriptions limit 1000");
        ...
        if( $getShop ){
            ...
            if( $results["count"] ){
                  ...
                  $data = array("{SUBSCR_NAME}"=>$getShop["clients_shops_title"],
                                ...
                                "{SUBSCR_ALL_LINK}"=>$Shop->link( $getShop["clients_shops_id_hash"] ),
                                "{UNSUBCRIBE}"=>"",
...
$getOrderMessages = getAll("select * from uni_clients_orders_messages where clients_orders_messages_status=0 and clients_orders_messages_notification=0 group by clients_orders_messages_from_id_user");
...
if(count($getOrderMessages)){
   ...
   foreach ($getOrderMessages as $key => $value) {
      ...
      if($getUser){
          $data = array("{ORDER_LINK}"=>_link( "order/" . $value["clients_orders_messages_id_order"] ),
                        "{USER_NAME}"=>$getUser["clients_name"],
                        "{UNSUBCRIBE}"=>"",
                        "{ORDER_ID}"=>$value['clients_orders_messages_id_order'],
                        "{EMAIL_TO}"=>$getUser["clients_email"]
                       );

          $Profile->userNotification( [ "mail"=>["params"=>$data, "code"=>"NEW_ORDER_MESSAGE", "email"=>$getUser["clients_email"]],"method"=>1 ] );
      }

      update("update uni_clients_orders_messages set clients_orders_messages_notification=? where clients_orders_messages_id_order=? and clients_orders_messages_to_id_user=?", [1,$value["clients_orders_messages_id_order"],$value["clients_orders_messages_from_id_user"]]);

   }

}

$getOrderMessages = getAll("select * from uni_clients_orders_messages where clients_orders_messages_status=0 and clients_orders_messages_notification=0 group by clients_orders_messages_to_id_user");
...
if(count($getOrderMessages)){
   ...
   foreach ($getOrderMessages as $key => $value) {
      ...
      update("update uni_clients_orders_messages set clients_orders_messages_notification=? where clients_orders_messages_id_order=? and clients_orders_messages_to_id_user=?", [1,$value["clients_orders_messages_id_order"],$value["clients_orders_messages_to_id_user"]]);
...
$getShops = getAll("select * from uni_clients_shops where clients_shops_time_validity < now() and clients_shops_notification=0");
...
if( count($getShops) ){
   foreach ($getShops as $key => $value) {
      ...
      if($getUser){
          $data = array("{SHOP_LINK}"=>'<a href="'.$Shop->link($value['clients_shops_id_hash']).'" >'.$value["clients_shops_title"].'</a>',
                       "{SHOP_NAME}"=>$value["clients_shops_title"],
                       "{USER_NAME}"=>$getUser["clients_name"],
                       "{UNSUBCRIBE}"=>"",
                       "{EMAIL_TO}"=>$getUser["clients_email"]
                       );
    
          email_notification( array( "variable" => $data, "code" => "SHOP_END" ) );
      }

      update("update uni_clients_shops set clients_shops_notification=? where clients_shops_id=?", [ 1,$value["clients_shops_id"] ]);
