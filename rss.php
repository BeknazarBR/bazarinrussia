        ...
        foreach ($data["all"] as $key => $value) {
            ...
            if( !$turbo ){
				...
				<item>
					...
					<pubDate>' . date(DATE_RFC822, $value['blog_articles_date_add']) . '</pubDate>
				</item>'; 
				...
				<item turbo="true">		
					<link>' . $Blog->aliasArticle( $value ) . '</link>
					<category>'.$value['blog_category_name'].'</category>
					<pubDate>'.date(DATE_RFC822, strtotime($value['blog_articles_date_add'])).'</pubDate>
					<turbo:content>
						<![CDATA[
							<header>
		...
		foreach ($data["all"] as $value) {
            ...
            if( !$turbo ){
				...
				<item>
					...
					<pubDate>' . date(DATE_RFC822, strtotime($value['ads_datetime_add'])) . '</pubDate>
				</item>';
